    ...
    <div className="space-y-6">
      ...
      {simpleStats && simpleStats.total > 0 && (
        <>
          ...
          {/* Charts simplifiés: Amétropies + Acuité visuelle (zoomables) */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="cursor-pointer group relative" onClick={() => setExpandedCard('simple-ametropies')}>
            ...
            <div className="cursor-pointer group relative" onClick={() => setExpandedCard('simple-acuite')}>
              <div className="absolute top-3 right-3 z-10 opacity-0 group-hover:opacity-100 transition-opacity bg-white/80 dark:bg-neutral-700/80 rounded-lg p-1.5">
                <Maximize2 size={14} className="text-neutral-500 dark:text-neutral-400" />
              </div>
          ...
          >
            ...
            <div className="mb-6">
              <h2 className="text-lg font-bold text-neutral-800 dark:text-neutral-100">
                ...
                {expandedCard === 'simple-ametropies' && 'Amétropies – Bilans simplifiés'}
                {expandedCard === 'simple-acuite' && 'Acuité visuelle – Bilans simplifiés'}
              </h2>
              <p className="text-sm text-neutral-400 dark:text-neutral-500 mt-1">
                ...
                {expandedCard === 'simple-ametropies' && 'Répartition des amétropies issues des bilans simplifiés'}
                {expandedCard === 'simple-acuite' && "Distribution des acuités visuelles relevées (dépistage rapide)"}
              </p>
            </div>
            ...
            {/* Bilans simplifiés – Amétropies */}
            {expandedCard === 'simple-ametropies' && simpleStats && (
              <ResponsiveContainer width="100%" height={480}>
                <PieChart>
                  <Pie
                    data={Object.entries(simpleStats.ametropies || {}).map(([name, value]) => ({ name, value }))}
            ...
            {/* Bilans simplifiés – Acuité visuelle */}
            {expandedCard === 'simple-acuite' && simpleStats && (
              <ResponsiveContainer width="100%" height={480}>
                <PieChart>
                  <Pie
                    data={Object.entries(simpleStats.acuite_visuelle || {}).map(([name, value]) => ({ name, value }))}
